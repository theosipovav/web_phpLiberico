...
class GeneratorPdf
{
    ...
    protected function genPdfOfPath($path)
    {
        ...
        {
            echo '<h1>Файл имеет неверный формат</h1>';
            echo '<p>'.'Файл: ' . $path.'</p>';
        }
    }
    ...
    protected function genPdfOfServiceApp($id, $user, $db)
    {
        ...
        //@$result = socket_connect($socket, '192.168.33.188', 15150);
        if ($result === false)
        {
            echo '<h1>Ошибка SOCKET-соеденения</h1>';
            echo '<h1>Ошибка SOCKET-соеденения</h1>';
            echo '<p>socket_connect($socket, "192.168.233.238", 11001)</p>';
            return;
        }
